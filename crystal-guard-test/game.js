...
// --- UPDATED FIREBASE LEADERBOARD FUNCTIONS ---

async function submitScore() {
    const nameInput = document.getElementById('player-name');
    const name = nameInput.value.trim() || "Anonymous"; // Handle empty names
    ...
    try {
        // Add to Firestore collection "leaderboard"
        await addDoc(collection(db, "leaderboard"), {
            name: name,
            score: parseInt(gameState.score),
            wave: parseInt(gameState.wave),
            timestamp: Date.now() // for future sorting if needed
        });
...
async function fetchLeaderboard() {
    ...
    try {
        // Query Firestore: collection 'leaderboard', sort by 'score' desc, top 10
        const q = query(
...
// This is critical because the script is now a Module.
// Without this, onclick="startGame()" in HTML won't find the function.
